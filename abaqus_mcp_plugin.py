...
def _resolve_mcp_home():
    """Resolve MCP home directory.

    Priority:
    1. ABAQUS_MCP_HOME environment variable (explicit override)
    2. <workspace>/mcp_home — workspace = directory containing this plugin file

    No fallback to ~/.abaqus-mcp.  plugin / client / mcp_server all resolve
    to the same mcp_home under the workspace when no env var is set.
    """
    env_home = os.environ.get('ABAQUS_MCP_HOME', '').strip()
    if env_home:
        return os.path.abspath(os.path.expanduser(env_home))
    # Default: <workspace>/mcp_home, where workspace = this file's directory.
